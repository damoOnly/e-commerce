...
namespace EcShop.UI.Web.Admin
{
	public class FileManagerJson : AdminPage
	{
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			{
				text = string.Format("{0}/fckfiles/Files/Image/", HiContext.Current.GetSkinPath());
		...
		public void FillTableForDb(string cid, string order, System.Collections.Hashtable table)
		{
			...
			System.Data.Common.DbCommand sqlStringCommand = database.GetSqlStringCommand("select CategoryId,CategoryName from Ecshop_PhotoCategories order by DisplaySequence");
			...
			if (cid.Trim() == "-1")
			{
				sqlStringCommand.CommandText = string.Format("select * from Ecshop_PhotoGallery order by {1}", cid, order);
			}
			else
			{
				sqlStringCommand.CommandText = string.Format("select * from Ecshop_PhotoGallery where CategoryId={0} order by {1}", cid, order);
			}
