...
namespace EcShop.UI.Web.Admin.WAPShop
{
	...
	public class WAPmanagehotkeywords : AdminPage
	{
		...
		private void btnSubmitHotkeyword_Click(object sender, System.EventArgs e)
		{
			...
			for (int i = 0; i < array2.Length; i++)
			{
                ...
                if (this.dropCategory.SelectedValue.HasValue)
                {
                    ...
                    {
                        StoreHelper.AddHotkeywords(this.dropCategory.SelectedValue.Value, text2, ClientType.WAP, HiContext.Current.User.UserId);
                        num++;
                    }
                }
                else
                {
                    ...
                    {
                        StoreHelper.AddHotkeywords(null, text2, ClientType.WAP, HiContext.Current.User.UserId);
                        num++;
                    }
                }
			}
		...
		{
            this.grdHotKeywords.DataSource = StoreHelper.GetHotKeywords(ClientType.WAP);
			this.grdHotKeywords.DataBind();
		}
		private bool IsSame(string word, int? categoryId)
		{
            System.Data.DataTable hotKeywords = StoreHelper.GetHotKeywords(ClientType.WAP);
