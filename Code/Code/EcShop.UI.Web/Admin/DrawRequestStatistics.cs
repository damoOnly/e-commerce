...
namespace EcShop.UI.Web.Admin
{
	...
	public class DrawRequestStatistics : AdminPage
	{
		...
		private void btnCreateReport_Click(object sender, System.EventArgs e)
		{
            ...
            {
                this.Page.Response.AppendHeader("Content-Disposition", "attachment;filename=会员提现报表.csv");
            }
            else
            {
                this.Page.Response.AppendHeader("Content-Disposition", "attachment;filename=会员提现报表_" + this.dateStart.Value.ToString("yyyyMMdd") + "-" + this.dateEnd.Value.ToString("yyyyMMdd") + ".csv");
            }
            ...
            EventLogs.WriteOperationLog(Privilege.MemberAccount, string.Format(CultureInfo.InvariantCulture, "用户{0},会员为{1}生成提现报告成功", new object[]
			{
