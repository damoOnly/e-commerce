...
{
    [PrivilegeCheck(Privilege.SupplierHomeTopicSetting)]
    public class SupplierHomeTopicSetting : AdminPage
    {
        ...
        {
            if (VShopHelper.RemoveHomeTopic((int)this.grdHomeTopics.DataKeys[e.RowIndex].Value, ClientType.VShop, supplierId))
            {
        ...
        {
            this.grdHomeTopics.DataSource = VShopHelper.GetHomeTopics(ClientType.VShop, supplierId);
            this.grdHomeTopics.DataBind();
        }
        private void btnDeleteAll_Click(object sender, System.EventArgs e)
        {
            if (VShopHelper.RemoveAllHomeTopics(ClientType.VShop, supplierId))
            {
                base.Response.Redirect(System.Web.HttpContext.Current.Request.Url.ToString(), true);
            }
        }
        protected void btnFinish_Click(object sender, System.EventArgs e)
        {
            foreach (System.Web.UI.WebControls.GridViewRow gridViewRow in this.grdHomeTopics.Rows)
            {
                ...
                if (int.TryParse(textBox.Text.Trim(), out displaysequence))
                {
                    ...
                    VShopHelper.UpdateHomeTopicSequence(topicId, displaysequence, ClientType.VShop);
                }
            }
            this.BindHomeProducts();
        }
