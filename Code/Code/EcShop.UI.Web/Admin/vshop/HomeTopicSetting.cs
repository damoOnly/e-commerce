...
namespace EcShop.UI.Web.Admin.vshop
{
	...
	public class HomeTopicSetting : AdminPage
	{
		...
		{
			if (VShopHelper.RemoveHomeTopic((int)this.grdHomeTopics.DataKeys[e.RowIndex].Value, ClientType.VShop))
			{
		...
		{
			this.grdHomeTopics.DataSource = VShopHelper.GetHomeTopics(ClientType.VShop);
			this.grdHomeTopics.DataBind();
		}
		private void btnDeleteAll_Click(object sender, System.EventArgs e)
		{
			if (VShopHelper.RemoveAllHomeTopics(ClientType.VShop))
			{
				base.Response.Redirect(System.Web.HttpContext.Current.Request.Url.ToString(), true);
			}
		}
