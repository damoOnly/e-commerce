	...
	public class BatchPrintSendOrderGoods : AdminPage
	{
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			foreach (OrderInfo current in this.GetPrintData(orderIds))
			{
				...
				{
					stringBuilder.AppendFormat("<li><span>管理员手工加价：</span>{0}</li>", System.Math.Round(adjustedDiscount, 2));
				}
				else
				{
					stringBuilder.AppendFormat("<li><span>管理员手工减价：</span>{0}</li>", System.Math.Round(-adjustedDiscount, 2));
				}
