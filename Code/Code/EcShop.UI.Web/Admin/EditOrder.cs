...
namespace EcShop.UI.Web.Admin
{
	...
	public class EditOrder : AdminPage
	{
		...
		public int GetPoint(OrderInfo order)
		{
			...
			if ((order.GetTotal() - order.AdjustedFreight - order.AdjustedDiscount - order.PayCharge) * order.TimesPoint / masterSettings.PointsRate > 2147483647m)
			{
				result = 2147483647;
			}
			else
			{
				...
				{
					result = (int)System.Math.Round((order.GetTotal() - order.AdjustedFreight - order.AdjustedDiscount - order.PayCharge) * order.TimesPoint / masterSettings.PointsRate, 0);
				}
			}
			return result;
		}
		...
		private void grdProducts_RowDeleting(object sender, System.Web.UI.WebControls.GridViewDeleteEventArgs e)
		{
			...
			string sku = this.grdProducts.DataKeys[e.RowIndex].Value.ToString();
