...
namespace EcShop.UI.Common.Controls
{
	public sealed class ButtonManager
	{
		...
		public static IButton CreateHiImageButton(HiImageButton button)
		{
			if (button == null)
			{
				throw new ArgumentNullException("button", "The button parameter can not be null");
			}
			return new HiImageButtonWrapper(button);
		}
		public static IButton Create(Control cntrl)
		{
			...
			if (button == null)
			{
				...
				{
					if (cntrl is HiImageButton)
					{
						button = ButtonManager.CreateHiImageButton(cntrl as HiImageButton);
					}
					else
					{
