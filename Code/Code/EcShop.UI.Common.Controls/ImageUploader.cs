...
namespace EcShop.UI.Common.Controls
{
	public class ImageUploader : WebControl
	{
		...
		{
			this.Controls.Clear();
            string webResourceUrl = this.Page.ClientScript.GetWebResourceUrl(base.GetType(), "ImageUploader.images.upload.png");
			WebControl webControl = new WebControl(HtmlTextWriterTag.Div);
			string value = "background:url(" + webResourceUrl + ");background-repeat:no-repeat; background-position:20px -80px";
			...
			if (HiContext.Current.User.UserRole != UserRole.SiteManager || this.IsUploaded)
			{
			...
			if (this.Page.Header.FindControl("uploaderStyle") == null)
			{
                ...
                webControl6.Attributes.Add("href", this.Page.ClientScript.GetWebResourceUrl(base.GetType(), "ImageUploader.css.style.css"));
		...
		public override void RenderControl(HtmlTextWriter writer)
		{
			...
			{
                string script = string.Format("<script src=\"{0}\" type=\"text/javascript\"></script>", this.Page.ClientScript.GetWebResourceUrl(base.GetType(), "ImageUploader.script.upload.js"));
