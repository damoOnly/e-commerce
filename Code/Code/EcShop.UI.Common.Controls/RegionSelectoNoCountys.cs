...
namespace EcShop.UI.Common.Controls
{
    ...
    /// 不带县级
    /// </summary>
	public class RegionSelectoNoCountys : WebControl
	{
		...
		protected override void CreateChildControls()
		{
            ...
            this.Controls.Add(RegionSelectoNoCountys.CreateTitleControl(this.ProvinceTitle));
            this.ddlProvinces = this.CreateDropDownList("ddlRegions1");
            RegionSelectoNoCountys.FillDropDownList(this.ddlProvinces, RegionHelper.GetAllProvinces(), this.provinceId);
            this.Controls.Add(RegionSelectoNoCountys.CreateTag("<span>"));
			this.Controls.Add(this.ddlProvinces);
            this.Controls.Add(RegionSelectoNoCountys.CreateTag("</span>"));
            this.Controls.Add(RegionSelectoNoCountys.CreateTitleControl(this.CityTitle));
            this.ddlCitys = this.CreateDropDownList("ddlRegions2");
			if (this.provinceId.HasValue)
			{
                RegionSelectoNoCountys.FillDropDownList(this.ddlCitys, RegionHelper.GetCitys(this.provinceId.Value), this.cityId);
			}
            this.Controls.Add(RegionSelectoNoCountys.CreateTag("<span>"));
		...
		protected override void Render(HtmlTextWriter writer)
		{
			...
			{
				string script = string.Format("<script src=\"{0}\" type=\"text/javascript\"></script>", this.Page.ClientScript.GetWebResourceUrl(base.GetType(), "region.helper.js"));
		...
		private static void FillDropDownList(WebControl ddlRegions, Dictionary<int, string> regions, int? selectedId)
		{
			...
			{
                WebControl webControl = RegionSelectoNoCountys.CreateOption(current.ToString(CultureInfo.InvariantCulture), regions[current]);
