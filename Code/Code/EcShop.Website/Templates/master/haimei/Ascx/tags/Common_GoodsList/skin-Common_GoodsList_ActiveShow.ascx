    ...
    <ItemTemplate>
        <div class="main">
            <div class="topicbanner">
                <img src="<%# Eval("PCBannerImageUrl") %>" /></div>
            <div class="category_pro_list cp-list">
                <ul>
                    <asp:Repeater ID="Repeater2" runat="server" DataSource='<%# Eval("ProductItems") %>'>
                        <ItemTemplate>
              ...
              <Hi:ProductDetailsLink ID="ProductDetailsLink2" runat="server"  ProductName='<%# Eval("ProductName") %>'  ProductId='<%# Eval("ProductId") %>' ImageLink="true">
                ...
                <div class="prolist-discount" <%#Eval("IsDisplayDisCount").ToString().ToLower()=="false"?"style=\"display:none\"":"" %>  ><%# Eval("DiscountRate").ToString() %>折</div>
              ...
              <div class="category_pro_name"><Hi:ProductDetailsLink ID="ProductDetailsLink1" runat="server"  ProductName='<%# Eval("ProductName") %>'  ProductId='<%# Eval("ProductId") %>'></Hi:ProductDetailsLink></div>
                <img src="http://www.haimylife.com<%#Eval("Icon") %>" style="height:18px;"><%# string.IsNullOrWhiteSpace(Eval("ShopName").ToString())?Eval("SupplierName"):Eval("ShopName") %></a>
             <div class="bars fix">
             <%--<div class="pd-size">
