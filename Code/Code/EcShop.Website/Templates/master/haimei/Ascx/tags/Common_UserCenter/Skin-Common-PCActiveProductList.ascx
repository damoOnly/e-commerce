...
<%@ Register TagPrefix="Hi" Namespace="EcShop.UI.SaleSystem.Tags" Assembly="EcShop.UI.SaleSystem.Tags" %>
 <li>
              ...
              <div class="category_pro_price"><p>¥<strong><Hi:FormatedMoneyLabel ID="FormatedMoneyLabel2"  Money='<%# Eval("RankPrice") %>' runat="server" /></strong></p><%--<em>¥:<span>10</span></em>--%>
                ...
                <span>¥<Hi:FormatedMoneyLabel ID="FormatedMoneyLabel1"  Money='<%# Eval("SalePrice") %>' runat="server" /></span>
              </div>
              
              <div class="bars fix cart-tip">
              <span class="shipping"><%#Eval("ShippingMode") %></span>
             <span class="addtofav" onclick="AddToFav(<%#Eval("ProductId") %>)" style="cursor:pointer"><label id="<%#Eval("ProductId")%>"><%#Eval("CollectCount")%></label></span>
                 <div class="pd-size">
              ...
              <Hi:SKUSelector4List ID="SKUSelector" runat="server" ProductId='<%#Eval("ProductId") %>' cssclass="Product_WareFun_bg"/>
             	...
             	<div class="num-wrap fix" style="display:none;" >
                  ...
                  <a class='<%# Convert.ToInt32(Convert.IsDBNull(Eval("Stock"))?"0":Eval("Stock"))>0?"addcartButton cart-btn":"addcartButton cart-btn disabled" %>' productid="<%#Eval("ProductId") %>" price="<%# Eval("SalePrice", "{0:F2}") %>" fastbuy_skuid="<%# Eval("fastbuy_skuid") %>"></a>
              </div>
</li>