...
<li>
    <div class="odbox">
    	<a class="odbox-head fix" href="<%#Globals.ApplicationPath + "/vShop/MemberOrderDetails.aspx?OrderId=" + Eval("OrderId") %>">
            <label class="fr ml5 odbox-date">申请时间：<span><%# Convert.ToDateTime(Eval("ApplyForTime")).ToString("yyyy-MM-dd")%></span></label><label class="odbox-ser">订单编号：<span><%# Eval("OrderId") %></span></label>
        </a>        
        <div class="odbox-con">
            ...
            <div class="odbox-info fix">                
                <div class="odbox-summer">
                    ...
                    处理状态：<%#Eval("HandleStatus").ToString()=="0"?"待处理":(Eval("HandleStatus").ToString()=="1"?"已完成":(Eval("HandleStatus").ToString()=="2"?"已拒绝":"已受理")) %>
                     ...
                     <div class="odbox-op fix">
                          <a href="MemberReplaceDetails.aspx?ReplaceId=<%#Eval("ReplaceId")%>" class="">查看</a>                         
                         </div>
                </div>
            </div>
        </div>
