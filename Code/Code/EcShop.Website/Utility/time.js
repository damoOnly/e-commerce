...
//倒数时间详细页面
function showTimeList(overdata, showObjId, isStart, startdata, nowTime) {
    if (!nowTime)
        nowTime = new Date();
    ...
    if (parseInt(startTotal) >= 0) {
        ...
        $("#" + showObjId).html(startdata.substring(0, startdata.length - 3) + '&nbsp;&nbsp;开抢');
        ...
        // 显示
        if (isStart == "1") {
            $("#" + showObjId).html("<span class='haveColor'>距结束：" + showTime + "</span>");
            $("#buyButton").show();
        }
        else
            $("#" + showObjId).html("距开始");
