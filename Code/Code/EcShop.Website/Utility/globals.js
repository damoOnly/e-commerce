...
window.onload = function () {
    $('span.page-skip :button').removeAttr('onclick').click(function () { goPage(this); });
    $('span.page-skip :text').keydown(function (e) {
        if (e.keyCode == 13) {
            var button = $(this).parent('span.page-skip').children(':button')[0];
            $(button).click();
...
function goPage(obj)
{
    var button = obj || (event.srcElement || event.target);
    var reg = /^[0-9]*[1-9][0-9]*$/;
    var inputPageCount = $(button).parent('span.page-skip').children(':text').val();
    if (!reg.test(inputPageCount)) {
        alert("页码只能输入正整数");
        inputPageCount = 1;
    }

    var pageText = $(button).parent('span.page-skip').text();
    var pageCount = pageText.substring(pageText.indexOf('/') + 1, pageText.indexOf('页'));
    inputPageCount = parseInt(inputPageCount);
    if (inputPageCount > pageCount)
    {
        alert('页码数最大为' + pageCount);
        inputPageCount = pageCount;
    }

    var url = AppendParameter('pageindex', inputPageCount);
    location.href = url;
}
$(function () {
    var h = $(".guild-wrap").height(), w = $(".guild-wrap").width(), ww = $(window).width();
    $(window).scroll(function () {
        var th = $(this).scrollTop();
        if (th > h) {
            $(".guild-wrap").css({ position: 'fixed',width:'100%',zIndex:'999', left: -(ww - w) / 2, top: 0 })
        } else {
            $(".guild-wrap").css({ position: 'static', width: '100%', zIndex: '999', left:'auto', top: 0 })
        }
    })
    //关闭 头部 广告
