...
function PageIsValid()
{
    ...
    {
        var txtval = $("#" + this["id"]);
        if (txtval.val() == undefined || txtval.val() == "" || txtval.val() == null)
        {
 
            isValid = false;
            var tiptx = $("#" + this["id"] + "Tip");
            txtval.addClass("txtborder");
            var txttip = tiptx.addClass("msgError");
            var tipX = tiptx.attr("id");
            if (tipX != undefined && tipX != "" && tipX != null) {
                switch (tipX) {
                    case "SubmmitOrder_txtShipToTip":
                        txttip.html("收货人名字只能为2-6个汉字");
                        break;
                    case "SubmmitOrder_txtAddressTip":
                        txttip.html("详细地址不能为空,3-200个字符");
                        break;
                    case "SubmmitOrder_txtCellPhoneTip":
                        txttip.html("手机号码格式不正确");
                        break;
                    
                }
            }
        }
        else if ($("#" + this["id"]).get(0).validator != undefined && $("#" + this["id"]).get(0).validator != null) {
            for (var i = 0; i < $("#" + this["id"]).get(0).validator.length; i++)
            {
                if ($("#" + this["id"]).get(0).validator[i]._IsValid == false)
                {
                    ...
                    $("#" + this["id"]).addClass("txtborder");
                    isValid = false;
                } else {
                    $("#" + this["id"]).removeClass("txtborder");
                }
            }
        }
    });
...
function PageIsValidAddress() {
    ...
    ctls.each(function () {
        ...
        if (txtval.val() == undefined || txtval.val() == "" || txtval.val() == null) {
            ...
            if (tipX != undefined && tipX != "" && tipX != null) {
                switch (tipX) {
                    case "userShippingAddresses_txtShipToTip":
                        txttip.html("收货人名字只能为2-6个汉字");
                        break;
                    case "userShippingAddresses_txtAddressTip":
                        txttip.html("详细地址不能为空,长度限制在3-60个字符之间");
                        break;
                    case "userShippingAddresses_txtCellPhoneTip":
                        txttip.html("手机号码格式不正确");
                        break;
