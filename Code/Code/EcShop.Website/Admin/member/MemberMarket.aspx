<%@ Page Language="C#" MasterPageFile="~/Admin/Admin.Master" AutoEventWireup="true" CodeBehind="MemberMarket.aspx.cs" Inherits="EcShop.UI.Web.Admin.MemberMarket" %>
		  ...
		  <div class="batchHandleArea">
		    <ul>
              ...
              <span class="delete"><Hi:ImageLinkButton ID="lkbDelectCheck1" IsShow="true" Height="25px" runat="server" Text="删除" /></span>
               <span id="Span4" class="sendmsg"  runat="server"><a href="javascript:void(0);" onclick="return SendMessage()">短信群发</a></span>
              <span id="Span5" class="sendemail" runat="server"><a href="javascript:void(0);" onclick="return SendEmail()">邮件群发</a></span>
...
<script>
    ...
    function updateLiteralText() {
        if (getQueryString("type") == "new")
        { $(".title strong:eq(0)").text('新客户') }
        else if (getQueryString("type") == "activy")
        { $(".title strong:eq(0)").text('活跃客户') }
        else
        { $(".title strong:eq(0)").text('休眠客户') }
    }
    $(document).ready(function () { updateLiteralText(); });
</script>
</asp:Content>