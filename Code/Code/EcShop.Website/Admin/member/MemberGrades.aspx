<%@ Page Language="C#" MasterPageFile="~/Admin/Admin.Master" AutoEventWireup="true" CodeBehind="MemberGrades.aspx.cs" Inherits="EcShop.UI.Web.Admin.MemberGrades" %>
...
<span>使用会员等级区分买家的级别，不同级别的买家可以享受不同的折扣率.</span></div>
	  ...
	  <!--数据列表区域-->
  <div class="datalist">
    <UI:Grid ID="grdMemberRankList" runat="server" AutoGenerateColumns="false" ShowHeader="true" DataKeyNames="GradeId" GridLines="None" Width="100%" EmptyDataText=" " HeaderStyle-CssClass="table_title">
              <Columns>
                  ...
                  <asp:TemplateField HeaderText="备注" ItemStyle-Width="28%" HeaderStyle-CssClass="td_right td_left">
                        <ItemTemplate>
                           <asp:Literal ID="litDescription" Text='<%# String.IsNullOrEmpty(Eval("Description").ToString())?"&nbsp;":Eval("Description") %>'  runat="server"></asp:Literal>
                        </ItemTemplate>
                  </asp:TemplateField>
                  <asp:TemplateField HeaderText="操作" HeaderStyle-Width="22%" HeaderStyle-CssClass="td_right td_left">
                        <ItemStyle CssClass="spanD spanN" />
                        <ItemTemplate>
                             <span class="submit_bianji"><asp:HyperLink ID="lkbViewAttribute" runat="server" Text="编辑" NavigateUrl='<%# "EditMemberGrade.aspx?GradeId="+Eval("GradeId")%>' ></asp:HyperLink></span>
