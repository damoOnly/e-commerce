...
function ResetGroupBuyProducts() {
    ...
    var supplierid = $("#ctl00_contentHolder_ddlSupplier").val();
    var postUrl = "addgroupbuy.aspx?isCallback=true&action=getGroupBuyProducts&timestamp=";
    postUrl += new Date().getTime() + "&categoryId=" + categoryId + "&sku=" + encodeURI(sku) + "&productName=" + encodeURI(productName) + "&supplierId=" + encodeURI(supplierid);
...
function FillProducts(product) {
    ...
    $.each(product, function(i, product) {
        productSelector.append(String.format("<option value=\"{0}\">{1}<\/option>", product.ProductId, decodeURIComponent(product.ProductName)));
    });
}
