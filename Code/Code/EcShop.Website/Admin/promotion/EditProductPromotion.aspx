<%@ Page Language="C#" MasterPageFile="~/Admin/Admin.Master" AutoEventWireup="true" CodeBehind="EditProductPromotion.aspx.cs" Inherits="EcShop.UI.Web.Admin.EditProductPromotion" Title="无标题页" %>
    ...
    <script type="text/javascript" language="javascript">
        ...
        function SelectPromoteType() {
            ...
            if (promoteType == 1)
                ...
                $("#radPromoteType_FullCut").attr("checked", true);
            else if (promoteType == 8)
                $("#radPromoteType_DoubleReduced").attr("checked", true);
            else if (promoteType == 9)
                $("#radPromoteType_DoubleDiscount").attr("checked", true);
            else if (promoteType == 10)
                $("#radPromoteType_PresentProduct").attr("checked", true);
        }
        ...
        function ShowPromotion(isClick) {
            ...
            else if ($("#radPromoteType_SentProduct").is(':checked') || $("#radPromoteType_PresentProduct").is(':checked')) {
                $("#liPromoteTypeDiscount").show();
                $("#spCondition").show();
                $("#spDiscountValue").show();
                $("#lblConditionTip").html("购买(件)：");
            ...
            else if ($("#radPromoteType_FullCut").is(':checked')) {
                $("#liPromoteTypeDiscount").show();
                $("#spCondition").show();
                $("#spDiscountValue").show();
                $("#lblConditionTip").html("满(元)：");
                $("#lblDiscountValueTip").html("减(元)：");
                $("#spIsAscend").hide();
            }

            else if ($("#radPromoteType_DoubleReduced").is(':checked')) {
                $("#liPromoteTypeDiscount").show();
                $("#spCondition").hide();
                $("#spDiscountValue").show();
                $("#lblDiscountValueTip").html("立减金额(元)：");
            ...
            else if ($("#radPromoteType_DoubleDiscount").is(':checked')) {
                $("#liPromoteTypeDiscount").show();
                $("#spCondition").hide();
                $("#spDiscountValue").show();
                $("#lblDiscountValueTip").html("折扣值(如果打9折，请输入0.9)：");
        ...
        function Valid() {
            ...
            if (promoteType == 1) {
            ...
            else if (promoteType == 6 || promoteType = 10) {
                if (condition.length == 0) {
                    alert("请输入购买数量！");
                    return false;
                }
                ...
                if (!numexp.test(discountValue)) {
                    alert("输入赠送数量(件)有误(必须是正数)，请重新输入正确的赠送数量！");
                    return false;
                }
            }
                ...
                if (!exp.test(condition)) {
                    alert("输入满足金额(元)有误(必须是数值)，请重新输入正确的满足金额！");
                    return false;
                }
                ...
                if (!exp.test(discountValue)) {
                    alert("输入减少金额(元)有误(必须是数值)，请重新输入正确的减少金额！");
                    return false;
                }
            }
                ...
                <div class="formitem validator4">
                    <ul>
                        ...
                        <li id="liPromoteTypeDiscount" style="display: none;">
                            ...
                            <table>
                                <tr>
                                    <td><span id="spCondition"><span id="lblConditionTip" class="formitemtitle Pw_110"></span>
                                        <asp:TextBox ID="txtCondition" runat="server" CssClass="forminput"></asp:TextBox></span>　　</td>
                                    <td><span id="spDiscountValue"><span id="lblDiscountValueTip" class="formitemtitle Pw_110" style="width: 230px;"></span>
                                        <asp:TextBox ID="txtDiscountValue" runat="server" CssClass="forminput"></asp:TextBox></span></td>
                                    <td> <span id="spIsAscend" style="display:none;"><span id="lblIsAscendValueTip" class="formitemtitle Pw_110" style="width:230px;">是否递增：</span><asp:RadioButton ID="radYes" runat="server"  Text="递增"  GroupName="ascend"/>
        <asp:RadioButton ID="radNo" runat="server" Text="不递增" Checked="true" GroupName="ascend" /></span></td>
                                </tr>
                            </table>
                    ...
                    <ul class="btntf Pa_110 clear" style="margin-left: 8px;">
                        <asp:Button ID="btnNext" runat="server" Text="保存修改" OnClientClick="return Valid();" CssClass="submit_jixu inbnt m_none" />
                    </ul>
                </div>
            </div>
        </div>
