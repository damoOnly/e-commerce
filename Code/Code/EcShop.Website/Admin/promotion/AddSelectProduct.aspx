 <%@ Page Language="C#" MasterPageFile="~/Admin/Admin.Master" AutoEventWireup="true" CodeBehind="AddSelectProduct.aspx.cs" Inherits="EcShop.UI.Web.Admin.AddSelectProduct" Title="无标题页" %>
...
<script language="javascript" type="text/javascript">
    ...
    function AddBindProduct() {
        ...
        $(chks).each(function (i, item) {
            ...
            //    tr += String.format("<tr name='appendlist'><td>{0}</td><td >{2}</td><td>{3}</td><td style='display:none'>{4}</td><td ><span  id='{1}' style='cursor:pointer;color:blue' onclick='Remove(this)'>删除</span></td></tr>", proname, arr[0], arr[1], arr[2], arr[3] + "|" + arr[0]);
            if ($listparent.find("span[id='" + arr[3] + "']").length == 0)
                tr += String.format("<tr name='appendlist'><td>{0}</td><td style='display:none'>{1}</td><td ><span  id='{1}' style='cursor:pointer;color:blue' onclick='Remove(this)'>删除</span></td></tr>", proname, arr[3]);
        });
