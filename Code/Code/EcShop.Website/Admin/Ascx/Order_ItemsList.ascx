<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="Order_ItemsList.ascx.cs" Inherits="EcShop.UI.Web.Admin.Order_ItemsList" %>
    ...
    <asp:DataList ID="dlstOrderItems" runat="server"  Width="100%" >
      ...
      <table width="200" border="0" cellspacing="0">
	    <tr class="table_title">
	      ...
	      <td width="11%" class="td_right td_left">商品单价(元) </td>
	      <td width="10%" class="td_right td_left">购买数量 </td>
	      <td width="11%" class="td_right td_left">发货数量 </td>
          <td width="7%" class="td_right td_left">税率 </td>
            <td width="8%" class="td_right td_left">供货商 </td>
	      <td width="11%" class="td_left td_right_fff">小计(元) </td>
        </tr>
        </HeaderTemplate>
        <ItemTemplate>        
          <tr>
	      ...
	      <td width="32%"><span class="Name"><a href='<%#"../../ProductDetails.aspx?ProductId="+Eval("ProductId") %>' target="_blank">
	        <%# Eval("ItemDescription")%></a></span> <span class="colorC">货号：<asp:Literal runat="server" ID="litCode" Text='<%#Eval("sku") %>' />
                <a href="javascript:updateSku('<%#Eval("OrderId") %>','<%#Eval("SkuId") %>',<%#Eval("ProductId") %>,'<%#Eval("sku") %>');" class="orderitem">[修改]</a>
          ...
          <td><asp:Literal runat="server" ID="litTaxRate" Text='<%#Eval("TaxRate") %>' /></td>
            <td><asp:Literal runat="server" ID="litSupplierName" Text='<%#Eval("SupplierName").ToString()==""?"  - ":Eval("SupplierName") %>' />
                <a href="javascript:updateSupplier('<%#Eval("OrderId") %>','<%#Eval("SkuId") %>',<%#Eval("ProductId") %>,<%#Eval("SupplierId") %>);" class="orderitem">[修改]</a>
            </td>
	      <td>
	        <div class="color_36c">
