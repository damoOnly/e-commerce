<%@ Page Title="" Language="C#" MasterPageFile="~/Admin/Admin.Master" AutoEventWireup="true"
    CodeBehind="StoreManageOrder.aspx.cs" Inherits="EcShop.UI.Web.Admin.StoreManageOrder" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="contentHolder" runat="server">
    <div>
        <img src="../images/lightbulb.png" /><literal>【门店订单管理】此处的订单编号查询不受订单状态限制，历史订单列表只显示三个月前的订单</literal>
    </div>
    <div style="margin-bottom: 10px;"><a href="AddStoreOrder.aspx" runat="server" class="submit_jia ibtn">添加门店订单</a></div>
            ...
            <ul>
                ...
                <li><span>门店名称：</span><span>
                    <abbr class="formselect">
                        <asp:DropDownList runat="server" ID="dllStoremanage" Width="107" />
                    </abbr>
                </span>
                </li>
            ...
            <asp:DataList ID="dlstOrders" runat="server" DataKeyField="OrderId" Width="100%">
                ...
                <ItemTemplate>
                    ...
                    <tr>
                        ...
                        <td style="color: #0B5BA5">&nbsp;
                            <Hi:OrderStatusLabel ID="lblOrderStatus" OrderStatusCode='<%# Eval("OrderStatus") %>' runat="server" SourceOrderIdCode='<%# Eval("SourceOrderId") %>' />
              ...
              <a style="color:Red" href="javascript:DialogFrame('<%# "sales/SendOrderGoods.aspx?OrderId="+ Eval("OrderId") %>','订单发货',null,null, function () {  location.reload(); })">发货</a> </asp:Label>
                                ...
                                <a style="color: Red"></a><a href="javascript:void(0)" onclick="return CheckRefund(this.title)"
                                    runat="server" id="lkbtnCheckRefund" visible="false" title='<%# Eval("OrderId") %>'>确认退款</a> <a href="javascript:void(0)" onclick="return CheckReturn(this.title)" runat="server"
    ...
    <script type="text/javascript">
        ...
        function printGoods() {
            ...
            if (orderIds == "") {
                ...
                $.ajax({
                    ...
                    data: { action: "CheckIsPrivilege", rightname: "StoreOrderPrintGoods" },
        ...
        function batchSend() {
            ...
            if (orderIds == "") {
                ...
                $.ajax({
                    url: "/admin/sales/OrderProcess.ashx",
                    type: 'post', dataType: 'json', timeout: 10000,
                    data: { action: "CheckIsPrivilege", rightname: "StoreOrderBatchSend" },
                    success: function (resultData) {
                        if (resultData.Status == "OK") {
        ...
        function batchSendByExcel() {
            ...
            $.ajax({
                url: "/admin/sales/OrderProcess.ashx",
                type: 'post', dataType: 'json', timeout: 10000,
                data: { action: "CheckIsPrivilege", rightname: "StoreOrderUploadGoods" },
                success: function (resultData) {
                    if (resultData.Status == "OK") {
                        DialogFrame("sales/BathSendOrderByExcel.aspx", "批量上传发货单", null, null, function () { location.reload(); });
                    }
                    else {
                        alert("没有批量上传发货单的权限");
                    }
        ...
        function printPosts() {
            ...
            if (orderIds == "") {
                ...
                $.ajax({
                    url: "/admin/sales/OrderProcess.ashx",
                    type: 'post', dataType: 'json', timeout: 10000,
                    data: { action: "CheckIsPrivilege", rightname: "StoreOrderPrintPosts" },
                    success: function (resultData) {
                        if (resultData.Status == "OK") {
