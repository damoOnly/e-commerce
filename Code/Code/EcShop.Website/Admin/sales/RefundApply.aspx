<%@ Page Title="" Language="C#" MasterPageFile="~/Admin/Admin.Master" AutoEventWireup="true"
    CodeBehind="RefundApply.aspx.cs" Inherits="EcShop.UI.Web.Admin.sales.RefundApply" %>
...
<asp:Content ID="Content3" ContentPlaceHolderID="contentHolder" runat="server">
                    ...
                    <ul>
                        <li class="batchHandleButton"><span class="signicon"></span><span class="allSelect">
                            ...
                            <span class="printorder excelOrder">
                                <a onclick="OrderRefund(this)">导出退款单</a></span>
                            <span class="printorder excelRefundDetails">
                                <a onclick="OrderRefundDetails(this)">导出退款详情</a></span>
                        </li>
                    </ul>
                </div>
            </div>
                ...
                <asp:DataList ID="dlstRefund" runat="server" DataKeyField="RefundId" Width="100%">
                    ...
                    <ItemTemplate>
                        <tr>
                            ...
                            <td style="word-wrap: break-word">&nbsp;<%# Eval("RefundRemark")%></td>
                            <td>
                            ...
                            <td>
                                ...
                                <a href="javascript:void(0)" onclick="DoReceive(this.title,this)" runat="server" id="lkbtnReceive" visible="false" title='<%# Eval("RefundId") %>'>领取</a>
                            </td>
                        </tr>
                    </ItemTemplate>
                    <FooterTemplate>
            ...
            <table cellpadding="0" cellspacing="0" width="100%" border="0" class="fram-retreat">
                ...
                <tr>
                    ...
                    <td align="left" class="bd_td">
                        <asp:Label ID="lblOrderTotal" runat="server" /></td>
                </tr>
                <tr>
                ...
                <tr>
                    ...
                    <td align="left" class="bd_td">&nbsp;<asp:TextBox ID="txtRefundTotal" runat="server" Width="100" onkeyup="clearNoNum(this);" /></td>
                </tr>
                <tr>
                ...
                <tr>
                    ...
                    <td align="left" class="bd_td">&nbsp;<asp:TextBox ID="txtAdminRemark" runat="server" Width="243" /></td>
                </tr>
            </table>
    ...
    <!--导出退款申请单-->
    <div id="OrderRefund" style="display: none;">
        <div class="frame-content" style="text-align: center;">
            <input type="button" id="btnSelExport" onclick="javascript: ExRefundOrders();" class="submit_DAqueding"
                value="选中单号导出" />
            &nbsp;
                <input type="button" id="btnTimeExport" onclick="javascript: ExRefundOrdersTime();" class="submit_DAqueding"
                    value="按时间段导出" />
            &nbsp;
        </div>
    </div>
    <div id="OrderRefundDetails" style="display: none;">
        <div class="frame-content" style="text-align: center;">
            <input type="button" id="btnSelExport" onclick="javascript: ExRefundOrdersDetails();" class="submit_DAqueding"
                value="选中单号导出" />
            &nbsp;
            <input type="button" id="btnTimeExport" onclick="javascript: ExRefundOrdersDetailsTime();" class="submit_DAqueding"
                value="按时间段导出" />
            &nbsp;
        </div>
    </div>
    ...
    <div style="display: none">
        ...
        <input type="hidden" id="hidIsAutoRefund" runat="server" />
        <input type="hidden" id="hidRefundTotal" runat="server" />
        ...
        <asp:Button ID="btnExcelOrderRefund" runat="server" CssClass="submit_DAqueding" Text="导出退款申请单(按选中的单)" />&nbsp;
        <asp:Button ID="btnExcelOrderRefundTime" runat="server" CssClass="submit_DAqueding" Text="导出退款申请单(按时间段)" />&nbsp;

        <asp:Button ID="btnExcelOrderRefundDetails" runat="server" CssClass="submit_DAqueding" Text="导出退款详情(按选中的单)" />&nbsp;
        <asp:Button ID="btnExcelOrderRefundDetailsTime" runat="server" CssClass="submit_DAqueding" Text="导出退款详情(按时间段)" />&nbsp;
    ...
    <script type="text/javascript">
        ...
        function clearNoNum(obj) {
            ...
            obj.value = obj.value.replace(/\.\d{3,}/g, ".");
            //保证.只出现一次，而不能出现两次以上
            obj.value = obj.value.replace(".", "$#$").replace(/\./g, "").replace("$#$", ".");
            $('#ctl00_contentHolder_hidRefundTotal').val(obj.value);
        ...
        function DoReceive(RefundId, ele) {
            var curr = $(ele);
            $.ajax({
                url: "/admin/sales/OrderProcess.ashx",
                type: 'post', dataType: 'json', timeout: 10000,
                data: { action: "OrderRefundReceive", RefundId: RefundId },
        ...
        //导出退款申请单
        function OrderRefund(elem) {
            var $elem = $(elem);
            var lefttop = $elem.offset();
            var dialog = ShowMessageDialogOrder("导出退款申请单", "orderRefund", "OrderRefund");
            lefttop.left = lefttop.left + $elem.outerWidth() + 2;
            dialog.DOM.wrap.css(lefttop);
        }

        /**导出退款申请单开始**/
        function ExRefundOrders() {
            var RefundIds = "";
            $("input:checked[name='CheckBoxGroup']").each(function (intde, domEle) {
            ...
            if (RefundIds == "") {
                ...
                $("#ctl00_contentHolder_btnExcelOrderRefund").trigger("click");
            }
        }
        function ExRefundOrdersTime() {
            var startTime = $('#ctl00_contentHolder_calendarStartDate').val();
            var endTime = $('#ctl00_contentHolder_calendarEndDate').val();
            if (startTime == '' || endTime == '') {
                alert("请选择时间区域");
            } else {
                $("#ctl00_contentHolder_btnExcelOrderRefundTime").trigger("click");
            }
        }
        /**导出退款申请单结束**/

        //导出退款详情信息
        function OrderRefundDetails(elem) {
            var $elem = $(elem);
            var lefttop = $elem.offset();
            var dialog = ShowMessageDialogOrder("导出退款详情", "OrderRefundDetails", "OrderRefundDetails");
            lefttop.left = lefttop.left + $elem.outerWidth() + 2;
            dialog.DOM.wrap.css(lefttop);
        }

        function ExRefundOrdersDetails() {
            var RefundIds = "";
            $("input:checked[name='CheckBoxGroup']").each(function (intde, domEle) {
                var tr = $(domEle).parent().parent().css("display");
                if (tr != "none") {
            ...
            if (RefundIds == "") {
                ...
                $("#ctl00_contentHolder_btnExcelOrderRefundDetails").trigger("click");
            }
        }
        function ExRefundOrdersDetailsTime() {
            var startTime = $('#ctl00_contentHolder_calendarStartDate').val();
            var endTime = $('#ctl00_contentHolder_calendarEndDate').val();
            if (startTime == '' || endTime == '') {
                alert("请选择时间区域");
            } else {
                $("#ctl00_contentHolder_btnExcelOrderRefundDetailsTime").trigger("click");
            }
        }
    </script>
</asp:Content>
