...
var qtyRow; // 销售库存
var factQtyRow;//商品库存(实际库存)
var weightRow; // 净重
var grossweightRow; //毛重
var deductFeeRow;//扣点
var ProductStandard;//商品规格，未开启多规格时使用
var selectCombinationValue;
...
$(document).ready(function() {
    ...
    factQtyRow = document.getElementById("factQtyRow");
    weightRow = document.getElementById("weightRow");
    deductFeeRow = document.getElementById("deductFeeRow");
    grossweightRow = document.getElementById("grossweightRow");
    ProductStandard = document.getElementById("ProductStandard");
    ...
    IsShowCombinationInfo();
});

function SetSaleTypeRead() {
    $("#ctl00_contentHolder_txtSalePrice").attr("readonly", "readonly");
    $("#ctl00_contentHolder_txtWeight").attr("readonly", "readonly");
    $("#ctl00_contentHolder_txtGrossWeight").attr("readonly", "readonly");
}

function RemoveSaleTypeRead() {
    $("#ctl00_contentHolder_txtSalePrice").removeAttr("readonly");
    $("#ctl00_contentHolder_txtWeight").removeAttr("readonly");
    $("#ctl00_contentHolder_txtGrossWeight").removeAttr("readonly");
}

function IsShowCombinationInfo()
{
...
function prepareControls(isReset) {
    ...
    $.ajax({
        ...
        error: function (XMLHttpRequest, textStatus, errorThrown) {
            alert(XMLHttpRequest.status + "-" + XMLHttpRequest.readyState + "-" + textStatus+"-"+postUrl);
        },
        ...
        if (attribute.UsageMode == "1") {
            ...
            if (attribute.AttributeValues.length > 0) {
                ...
                $.each(attribute.AttributeValues, function(vIndex, attributeValue) {
                    ...
                    var valItem = $("<span id=\"span_" + attribute.AttributeId + "\" class=\"valspan\" style=\"float:left\"><\/span>")
        ...
        success: function(data) {
            if (data.Status == "true") {
                ...
                if (userMode == "1") {
                    ...
                    $("#span_" + attributeId).append($(String.format("<input type=\"checkbox\" name=\"{0}\" id=\"{1}\" valueId=\"{2}\" checked=\"checked\" \/>", checkGroupName, cid, data.msg)));
                    $("#span_" + attributeId).append($(String.format("<label for=\"{0}\">{1}<\/label>", cid, valuename)));
                }
...
function restoreState() {
    ...
    if ($("#ctl00_contentHolder_chkSkuEnabled").attr("checked") == "checked") {
        ...
        $.each(selectedSkus, function() {
            ...
            $("#factQty_" + rowIndex).val($(this).attr("factQty"));
            $("#weight_" + rowIndex).val($(this).attr("weight"));
            $("#grossweight_" + rowIndex).val($(this).attr("grossweight"));
            var cosPrice = $(this).attr("costPrice");
            var deductFee = $(this).attr("deductFee");

            /*if (cosPrice) {
                $("#costPrice_" + rowIndex).val(cosPrice);
                $("#deductFee_" + rowIndex).hide();
                $("#deductFee_" + rowIndex).val('');
            }
            else if (deductFee) {
                $("#deductFee_" + rowIndex).val(deductFee);

                $("#costPrice_" + rowIndex).hide();
                $("#costPrice_" + rowIndex).val('');
            }
            else {
                $("#deductFee_" + rowIndex).val(deductFee);
                $("#deductFee_" + rowIndex).hide();
...
function setCtrlDisplay(displayCssStatus) {
    ...
    factQtyRow.style.display = displayCssStatus;
    weightRow.style.display = displayCssStatus;
    grossweightRow.style.display = displayCssStatus;
    enableSkuRow.style.display = displayCssStatus;
    deductFeeRow.style.display = displayCssStatus;
    ProductStandard.style.display = displayCssStatus;
}
...
function prepareSkus() {
    skuTable = $("<table width=\"98%\" border=\"0\" cellSpacing=\"0\" cellPadding=\"2\" class=\"SpecificationTable\"><\/table>");
    ...
    tableHeader.append($("<td align=\"center\" style=\"width:45px\">扣点<br\/><\/td>"));
    tableHeader.append($("<td align=\"center\" style=\"width:60px\"><em >*<\/em>销售库存<\/td>"));
    tableHeader.append($("<td align=\"center\" style=\"width:60px\"><em >*<\/em>商品库存<\/td>"));
    tableHeader.append($("<td align=\"center\" style=\"width:45px\">净重<br\/>(克)<\/td>"));
    tableHeader.append($("<td align=\"center\" style=\"width:45px\">毛重<br\/>(克)<\/td>"));
...
function showSkuValue() {
    ...
    for (index = 0; index < cellFields.length; index++) {
        ...
        if (!flag) {
            ...
            var btn = $(String.format("<input type=button id='btnadd_{0}' value='添加规格值'>", cellFields[index]));
            var ins = $(String.format("<span style='display:none;' id='sp_{0}'><input type=text id='txt_{0}' name='txt_{0}'><input type=button id='btnins_{0}' value='保存' onclick='addSkuValue({0})'></span>", cellFields[index]));
            $(btn).click(function() {
                $(this).next().show();
                $(this).hide();
            });
...
function removeSkuField(skuId, skuName, showConfirm) {
    if (skuCounts <= 1) { alert("请至少保留一个自定义规格项或者关闭规格。"); return false; }
...
function costPriceChange(ele)
{
    var currenele = $(ele);
    var salePrice = currenele.parent().parent().find(".skuItem_SalePrice").val();
    var deductele = currenele.parent().parent().find(".skuItem_DeductFee");
    if (isNaN(salePrice)) {
        return;
    }
    salePrice = parseFloat(salePrice);
    if (currenele.val()) {
        var costPrice = currenele.val();
        if (isNaN(costPrice)) {
            if (deductele != null) {
                deductele.val('');
            }
        ...
        if (deductele != null) {
            ...
            deductele.val(isNaN(fee) ? '' : fee.toFixed(2));
        }
    }
}

function deductFeeChange(ele) {
    var currenele = $(ele);
    var salePrice = currenele.parent().parent().find(".skuItem_SalePrice").val();
    var costPrice = currenele.parent().parent().find(".skuItem_CostPrice");
    if (isNaN(salePrice)) {
        return;
    }
    salePrice = parseFloat(salePrice);
    if (currenele.val()) {
        var deductFee = currenele.val();
        if (isNaN(deductFee)) {
            if (costPrice != null) {
                costPrice.val('');
            }
        ...
        if (costPrice != null) {
            ...
            costPrice.val(isNaN(price) ? '' : price.toFixed(2));
        }
    }
}
...
function addItem() {
    ...
    dataRow.append(createDeductFeeCell(newRowIndex));
    dataRow.append(createQtyCell(newRowIndex));
    dataRow.append(createFactQtyCell(newRowIndex));
    dataRow.append(createWeightCell(newRowIndex));
    dataRow.append(createGrossWeightCell(newRowIndex));
...
function createCostPriceCell(rowIndex) {
    var cell = createCell();
    var priceCell = $(String.format("<input type=\"text\" class=\"skuItem_CostPrice\" id=\"costPrice_{0}\" onkeyup=\"costPriceChange(this);\" onchange=\"costPriceChange(this);\" style=\"width:50px;\" \/>", rowIndex));
...
function createFactQtyCell(rowIndex) {
    var cell = createCell();
    var quantityCell = $(String.format("<input type=\"text\" class=\"skuItem_FactQty\" id=\"factQty_{0}\" style=\"width:35px;\" \/>", rowIndex));
    $(quantityCell).val($("#ctl00_contentHolder_txtFactStock").val());
    cell.append(quantityCell);
    return cell;
}
...
function createGrossWeightCell(rowIndex) {
    var cell = createCell();
    var grossweightCell = $(String.format("<input type=\"text\" class=\"skuItem_GrossWeight\" id=\"grossweight_{0}\" style=\"width:35px;\" \/>", rowIndex));
    $(grossweightCell).val($("#ctl00_contentHolder_txtGrossWeight").val());
    cell.append(grossweightCell);
    return cell;
}

function createDeductFeeCell(rowIndex) {
    var cell = createCell();
    var deductFeeCell = $(String.format("<input type=\"text\" class=\"skuItem_DeductFee\" id=\"deductFee_{0}\" onkeyup=\"deductFeeChange(this);\" onchange=\"deductFeeChange(this);\" style=\"width:35px;\" \/>", rowIndex));
    $(deductFeeCell).val($("#ctl00_contentHolder_txtDeductFee").val());
    cell.append(deductFeeCell);
    return cell;
}
...
function cancelValid() {
    ...
    $("#ctl00_contentHolder_txtFactStock").removeAttr("ValidateGroup");
    $("#ctl00_contentHolder_txtWeight").removeAttr("ValidateGroup");
    $("#ctl00_contentHolder_txtGrossWeight").removeAttr("ValidateGroup");
...
function reBindValid() {
    ...
    $("#ctl00_contentHolder_txtFactStock").attr("ValidateGroup", "default");
    $("#ctl00_contentHolder_txtWeight").attr("ValidateGroup", "default");
    $("#ctl00_contentHolder_txtGrossWeight").attr("ValidateGroup", "default");
...
function doSubmit() {
    ...
    if (skuEnabled) {
        if (skuCounts <= 0)
        {
            alert("请至少保留一个自定义规格项或者关闭规格。");
            return false;
        }
        // 商品规格数量需大于等于2
        if ($(".SpecificationTr").length < 1) {
            alert("开启规格以后，您至少需要增加1个商品规格！");
            return false;
        }
        ...
        if (!checkFactQty()) return false;
        if (!checkWeight()) return false;
        if (!checkGrossWeight()) return false;
        if (!checkDeductFee()) { return false;}
    }
...
function loadSkus() {
    ...
    $.each($(".SpecificationTr"), function(i, valitem) {
        ...
        var factQty = $("#factQty_" + rowIndex).val();
        var weight = $("#weight_" + rowIndex).val();
        var grossweight = $("#grossweight_" + rowIndex).val();
        var deductFee = $("#deductFee_" + rowIndex).val();
        var itemXml = String.format("<item skuCode=\"{0}\" salePrice=\"{1}\" costPrice=\"{2}\" qty=\"{3}\" factQty=\"{4}\" weight=\"{5}\" deductFee=\"{6}\" grossweight=\"{7}\">", skuCode, salePrice, costPrice, qty, factQty, weight, deductFee, grossweight);
        itemXml += "<skuFields>";
        for (i = 0; i < cellFields.length; i++) {
...
function checkCostPrice() {
    ...
    $('.skuItem_CostPrice').each(function () {
        var costPrice = $.trim($(this).val());
        var salePrice = $.trim($(this).parents('tr:first').find(':text.skuItem_SalePrice').val());
        var deductFee = $.trim($(this).parents('tr:first').find(':text.skuItem_DeductFee').val());
        if (costPrice == '' || deductFee == '') {
            alert('商品规格的成本价与扣点为必填项！');
            result = false;
            return false;
        }
        if (costPrice != '' && !exp.test(costPrice)) {
            alert('商品规格的成本价输入有误！');
            result = false;
            return false;
        }
        ...
        if (isNaN(costPrice) || costPrice <= 0 || costPrice >= salePrice) {
            alert('成本价必须是大于0小于一口价的数字！');
            result = false;
            return false;
        }
        if (isNaN(deductFee) || deductFee <= 0 || deductFee >= 1) {
            alert('商品规格中扣点必须是大于0小于1的数字！');
            result = false;
            return false;
        }
    });
...
function checkQty() {
    return checkNumber($(".skuItem_Qty"), true, "销售库存");
}

function checkFactQty() {
    return checkNumber($(".skuItem_FactQty"), true, "商品库存");
}

function checkWeight() {
    return checkPrice($(".skuItem_Weight"), false, "净重");
}

function checkGrossWeight() {
    return checkPrice($(".skuItem_GrossWeight"), false, "毛重");
}

function checkDeductFee() {
    return checkPriceDeductFee($(".skuItem_DeductFee"), false, "扣点");
}


function checkPriceDeductFee(inputItems, required, priceName) {
    var validated = true;
    var exp = new RegExp("^(0|(0+(\\.[0-9]{1,2}))|[1-9]\\d*(\\.\\d{1,2})?)$", "i");
