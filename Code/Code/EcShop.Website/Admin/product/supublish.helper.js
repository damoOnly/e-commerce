...
function GetCategories(parentCategoryId) {
    var categories = null;
    var postUrl = "SupplierSelectCategory.aspx?isCallback=true&action=getlist&timestamp=";
...
function GetIEVersion() {
    ...
    var regStr_ie = /rv:([\w.]+)/;
    var match = regStr_ie.exec(agent);
    if (agent.indexOf("msie") > -1 && match != null) {
        return match[1];
    }
    else
        return 0;
...
function GotoNext() {
    ...
    //form.method = "post";
    //form.action = "editproduct.aspx?categoryId=" + selectedCategoryId + "&productId=" + productId;
    //form.action = "addproduct.aspx?categoryId=" + selectedCategoryId;
    //        form.submit();
    if (window.event) {
    ...
    if (formpost) {
        ...
        if (productId != "" && productId.length > 0)
            form.action = "suppliereditproduct.aspx?categoryId=" + selectedCategoryId + "&productId=" + productId;
        else
            form.action = "supplieraddproduct.aspx?categoryId=" + selectedCategoryId;
        form.submit();
    }
    else {
        if (productId != "" && productId.length > 0)
            window.location.href = "suppliereditproduct.aspx?categoryId=" + selectedCategoryId + "&productId=" + productId;
        else {
            window.location.href = "supplieraddproduct.aspx?categoryId=" + selectedCategoryId;
        }
    }
...
$(document).ready(function () {
    ...
    nextButton.bind("click", function () { GotoNext(); });
...
function LoadState(categoryId) {
    $.ajax({
        url: String.format("SupplierSelectCategory.aspx?isCallback=true&action=getinfo&timestamp={0}&categoryId={1}", new Date().getTime(), categoryId),
        type: 'GET', dataType: 'json', timeout: 10000,
        async: false,
        success: function (resultData) {
            if (resultData.Status == "OK") {
