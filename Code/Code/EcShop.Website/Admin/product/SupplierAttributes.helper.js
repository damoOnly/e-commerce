...
function prepareControls(isReset) {
    if (currentTypeId.length == 0) currentTypeId = "0";
    var postUrl = "supplieraddproduct.aspx?isCallback=true&action=getPrepareData&timestamp=";
...
function createCostPriceCell(rowIndex) {
    var cell = createCell();
    var priceCell = $(String.format("<input type=\"text\" class=\"skuItem_CostPrice\" id=\"costPrice_{0}\" onchange=\"costPriceChange(this);\" style=\"width:50px;\" \/>", rowIndex));
...
function createDeductFeeCell(rowIndex) {
    var cell = createCell();
    var deductFeeCell = $(String.format("<input type=\"text\" class=\"skuItem_DeductFee\" id=\"deductFee_{0}\" onchange=\"deductFeeChange(this);\" style=\"width:35px;\" \/>", rowIndex));
