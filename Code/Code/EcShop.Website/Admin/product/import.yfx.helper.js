...
$(document).ready(function () {
    tbTypes = $("#tbTypes");
    infoZone = $("#infoZone");
    txtPTXml = $("#ctl00_contentHolder_txtPTXml");
    chkFlag = $("#ctl00_contentHolder_chkFlag");
    txtProductTypeXml = $("#ctl00_contentHolder_txtProductTypeXml");
    ptRow = $("#ptRow");
...
function fillTypeSelector(mappedTypeName, selector) {
    ...
    if ($.browser.msie) {
        ...
        xml.async = false;
        xml.loadXML($("#ctl00_contentHolder_txtProductTypeXml").val());
    }
    else {
        xml = new DOMParser().parseFromString($("#ctl00_contentHolder_txtProductTypeXml").val(), "text/xml");
    }
    var types = $(xml).find("item");
    if (types.length > 0) {
...
function doImport() {
    if ($("#ctl00_contentHolder_dropImportVersions").val().length == 0) {
        alert("请选择一个导入插件！");
        return false;
    }

    if ($("#ctl00_contentHolder_dropFiles").val().length == 0) {
        alert("请选择要导入的数据包文件！");
        return false;
    }

    if ($("#ctl00_contentHolder_dropCategories").val().length == 0) {
        alert("请选择要导入的商品分类！");
        return false;
    }

    if ($("#ctl00_contentHolder_dropProductLines").val().length == 0) {
        alert("请选择要导入的商品线！");
        return false;
    }
...
function loadDataTypes() {
    ...
    dataXml += "<\/types><\/xml>";
    $("#ctl00_contentHolder_txtMappedTypes").val(dataXml);
}