...
function ShowAddDiv() {
    DialogFrameClose("supplier/SupplierSearchBindProduct.aspx", "添加捆绑商品", null, null);
}
...
function CollectInfos() {
    ...
    if (trs.length < 2) {
        ...
        trs.each(function(i, item) {
            ...
            inputstr += $(item).children().eq(4).html() + "|" + parseInt(num) + ",";
        });
...
function GetSearchData(pageindex) {
    $.ajax({
        url: "SupplierAddBundlingProduct.aspx?isCallback=true&serachName=" + $("#serachName").val() + "&categoryId=" + $("#ctl00_contentHolder_dropCategories").val() +
