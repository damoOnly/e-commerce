...
{
	public class VshopAdminPage : AdminPage
	{
		protected override void Render(HtmlTextWriter writer)
		{
			string domainName = Globals.DomainName;
            string state = VshopAdminPage.PostData("http://vip.ecdev.cn/valid.ashx", "action=openvstore&product=2&host=" + domainName);
			int flag = Convert.ToInt32(state.Replace("{\"state\":\"", "").Replace("\"}", ""));
			if (flag != 1)
			{
				this.Page.Response.Redirect(Globals.GetAdminAbsolutePath("/AccessDenied.aspx?errormsg=您的微商城还暂未开通"), true);
			}
			base.Render(writer);
		}
