    ...
    public static class DataHelper
    {
        ...
        public static string CleanSearchString(string searchString)
        {
            ...
            else
            {
                ...
                //searchString = Regex.Replace(searchString, "delete|select|truncate ", " ", RegexOptions.Multiline | RegexOptions.Compiled| RegexOptions.IgnoreCase);
                //string regPattern = "(?:')|(?:--)|(/\\*(?:.|[\\n\\r])*?\\*/)|(\\b(select|update|and|or|delete|insert|trancate|char|into|substr|ascii|declare|exec|count|master|into|drop|execute)\\b)";
                string regPattern = @"<[^>]+?style=[\w]+?:expression\(|\b(alert|confirm|prompt)\b|^\+/v(8|9)|<[^>]*?=[^>]*?&#[^>]*?>|\b(and|or)\b.{1,6}?(=|>|<|\bin\b|\blike\b)|/\*.+?\*/|<\s*script\b|<\s*img\b|\bEXEC\b|UNION.+?SELECT|UPDATE.+?SET|INSERT\s+INTO.+?VALUES|(SELECT|DELETE).+?FROM|(CREATE|ALTER|DROP|TRUNCATE)\s+(TABLE|DATABASE)";
                searchString = Regex.Replace(searchString, regPattern, " ", RegexOptions.Multiline | RegexOptions.Compiled | RegexOptions.IgnoreCase);
                result = searchString;
            }
        ...
        {
            return DataHelper.PagingByRownumber(pageIndex, pageSize, sortBy, sortOrder, isCount, table, pk, filter, selectFields, 0, false);
        }
        public static DbQueryResult PagingByRownumberAdmin(int pageIndex, int pageSize, string sortBy, SortAction sortOrder, bool isCount, string table, string pk, string filter, string selectFields)
        {
            return DataHelper.PagingByRownumber(pageIndex, pageSize, sortBy, sortOrder, isCount, table, pk, filter, selectFields, 0, true);
        }
        ...
        public static long ConvertUniversalTime(DateTime time)//转换UNIX时间戳
        {
            return (time.ToUniversalTime().Ticks - 621355968000000000) / 10000000;
        }
        public static DateTime ConvertTimeFromUniversal(long unixTime)//UNIX时间戳转换datetime
        {
