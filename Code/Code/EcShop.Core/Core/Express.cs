			...
			if (!string.IsNullOrEmpty(text))
			{
				...
				if (xmlNode != null && xmlNode.ChildNodes.Count > 0)
				{
					foreach (XmlNode xmlNode2 in xmlNode)
					{
						...
						{
							string innerText = xmlNode2.Attributes["key"].InnerText;
						}
					}
				}
			}
			HttpWebRequest httpWebRequest = (HttpWebRequest)WebRequest.Create(string.Format("http://www.kuaidi100.com/query?type={0}&postid={1}", computer, expressNo));
