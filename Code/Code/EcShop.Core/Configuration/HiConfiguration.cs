...
using System.Xml;
namespace EcShop.Core.Configuration
{
	public class HiConfiguration
	{
		...
		private int shippingAddressQuantity = 20;
		...
		public static HiConfiguration GetConfig()
		{
			...
			if (hiConfiguration == null)
			{
				...
				if (current != null)
				{
					filename = current.Request.MapPath("~/config/Ecdev.config");
				}
				else
				{
                    filename = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "config\\Ecdev.config");
				}
