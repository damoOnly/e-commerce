...
{
	public class VDevelopReferrals : VMemberTemplatedWebControl
	{
		...
		private string CreateQRCode(string url, int userId)
		{
			...
			string text = this.Page.Request.MapPath(Globals.ApplicationPath + "/Storage/master/QRCode_VShop");
			if (System.IO.Directory.Exists(text))
			{
				string text2 = "referral_" + userId + ".png";
				bitmap.Save(text + "/" + text2, System.Drawing.Imaging.ImageFormat.Png);
				result = "/Storage/master/QRCode_VShop/" + text2;
			}
			else
			{
