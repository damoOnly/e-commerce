...
{
	public class AliOHDevelopReferrals : AliOHMemberTemplatedWebControl
	{
		...
		protected override void AttachChildControls()
		{
			...
			{
				string.Format(System.Globalization.CultureInfo.InvariantCulture, "{0}://{1}{2}/AliOH/Login.aspx", new object[]
				{
					url.Scheme,
			...
			string text2 = this.Page.Request.MapPath(Globals.ApplicationPath + "/Storage/master/QRCode_AliOH/");
			...
			})))
			{
				...
				{
					Globals.ApplicationPath,
					"/Storage/master/QRCode_AliOH/referral_",
					member.UserId,
					".png"
				});
				return;
			}
			if (this.linDevelopReferrals.HRef.IndexOf("/AliOH/Login.aspx") == -1)
			{
				this.imgQRCode.Src = this.CreateQRCode(this.linDevelopReferrals.HRef.Replace("/Login.aspx", "/AliOH/Login.aspx"), member.UserId);
				return;
			}
		...
		private string CreateQRCode(string url, int userId)
		{
			...
			string text = this.Page.Request.MapPath(Globals.ApplicationPath + "/Storage/master/QRCode_AliOH");
			if (System.IO.Directory.Exists(text))
			{
				string text2 = "referral_" + userId + ".png";
				bitmap.Save(text + "/" + text2, System.Drawing.Imaging.ImageFormat.Png);
				result = "/Storage/master/QRCode_AliOH/" + text2;
			}
			else
			{
